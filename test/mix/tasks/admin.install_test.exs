...
defmodule Mix.Tasks.ExAdmin.InstallTest do
  ...
  @css_files ~w(admin_lte2.css admin_lte2.css.map) ++
               @js_files(
  ...
  defp assert_webpack_assets(mode \\ :assert) do
    Enum.each(@js_files ++ @css_files, fn file ->
      assert_or_refute_file(mode, Path.join(~w(assets vendor) ++ [file]))
    end)

    Enum.each(@font_files, fn file ->
      assert_or_refute_file(mode, Path.join(~w(assets static fonts) ++ [file]))
    end)

    dest_path = ~w(assets static images ex_admin)
  ...
  defp assert_no_webpack_assets(mode \\ :assert) do
  ...
  def create_webpack_config do
    File.touch(Path.join(~w( assets webpack-config.js)))
  end
  ...
  test "installs with webpack" do
    in_tmp("installs with webpack", fn ->
      Logger.disable(self())
      create_config()
      ...
      assert_webpack_assets()

      assert_file("webpack-config.js", fn file ->
        assert file =~ Install.webpack_instructions()
      end)
    end)
  end

  test "no webpack install" do
    in_tmp("no webpack install", fn ->
      create_config()
      create_priv_static()

      Mix.Tasks.Admin.Install.run(["--no-webpack"])

      assert_file("config/config.exs", fn file ->
        assert file =~ "config :xain, :after_callback, {Phoenix.HTML, :raw}"
      end)

      assert_no_webpack_assets()
    end)
  end
  ...
  test "no assets install" do
    in_tmp("no assets install", fn ->
      ...
      assert_webpack_assets(:refute)
      assert_no_webpack_assets(:refute)
    end)
  end
